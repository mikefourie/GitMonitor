// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CommitRepository.cs">(c) 2021 Mike Fourie and Contributors (https://github.com/mikefourie/GitMonitor) under MIT License. See https://opensource.org/licenses/MIT</copyright>
// --------------------------------------------------------------------------------------------------------------------
namespace GitMonitor.Repositories
{
    ...
    public class CommitRepository
    {
        ...
        private static IEnumerable<Branch> ListBranchesContaininingCommit(Repository repo, string commitSha, string filter)
        {
        ...
        private MonitoredPath GetMonitoredPath(MonitoredPathConfig monitoredPathConfig, MonitoredPath monitoredPath, string repository, string branchName, DateTime startDateTime, DateTime endDateTime)
        {
            ...
            foreach (var dir in directoriesToScan)
            {
                try
                {
                    GitRepository gitrepo = TryGetRepo(monitoredPath, dir.Name);
                    using (Repository repo = new Repository(dir.FullName))
                    {
                        int commitCount = 0;
